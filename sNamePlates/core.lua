...
local function sNamePlates_CBSetValue(self, key)
    ...
    elseif key == "cast" then
		cchProgress = format("%.2f", ((endTime-startTime)/1000)-(endTime/1000 - GetTime()))
		self.castbar:SetValue(cchProgress)
		self.castbar.cbTime:SetText(cchProgress)
    end 
end
...
local function sNamePlates_UpdateFrame(self)
	...
	if sNamePlates.db.profile.castbarToggle then
		...
		elseif UnitCastingInfo("target") then
			...
			self.castbar:SetMinMaxValues(0,0)
		...
		if sNamePlates.db.profile.castbarToggle then
			if event == "PLAYER_TARGET_CHANGED" then
				if UnitChannelInfo("target") then	
					name, _, _, texture, startTime, endTime, _, castID, notInterruptible, spellID = UnitChannelInfo("target")
					if not notInterruptible then
						self:SetStatusBarColor(sNamePlates.db.profile.CBColorInterruptible.r, sNamePlates.db.profile.CBColorInterruptible.g, sNamePlates.db.profile.CBColorInterruptible.b, sNamePlates.db.profile.CBColorInterruptible.a)
					else
						self:SetStatusBarColor(sNamePlates.db.profile.CBColorNotInterruptible.r, sNamePlates.db.profile.CBColorNotInterruptible.g, sNamePlates.db.profile.CBColorNotInterruptible.b, sNamePlates.db.profile.CBColorNotInterruptible.a)
					end
					self:SetMinMaxValues(0, (endTime-startTime)/1000)
					frame.castbarIcon.Texture:SetTexture(texture)
					frame.castbar.cbName:SetText(name)
					self:Show()
				elseif UnitCastingInfo("target") then
					name, _, _, texture, startTime, endTime, _, castID, notInterruptible, spellID = UnitCastingInfo("target")
					if  not notInterruptible then
			...
			elseif (event == "UNIT_SPELLCAST_CHANNEL_START" or event == "UNIT_SPELLCAST_START") and arg1 == "target" then 
				if UnitChannelInfo("target") then	
					name, _, _, texture, startTime, endTime, _, castID, notInterruptible, spellID = UnitChannelInfo("target")
					if not notInterruptible then
						self:SetStatusBarColor(sNamePlates.db.profile.CBColorInterruptible.r, sNamePlates.db.profile.CBColorInterruptible.g, sNamePlates.db.profile.CBColorInterruptible.b, sNamePlates.db.profile.CBColorInterruptible.a)
...
function sNamePlates:RTextColors()
	...
	if sNamePlates.db.profile.castbarToggle or sNamePlates.db.profile.castbarFontToggle then
